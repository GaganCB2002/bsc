    ...
    <div className="lp-page" style={{ minHeight: '100vh' }}>
      ...
      <div className="container" style={{ padding: '40px 24px 80px' }}>
        <div style={{ marginBottom: '32px' }}>
          ...
          <h1 style={{ fontSize: '2.2rem', fontWeight: 300, color: '#1A1A2E', margin: 0 }}>
            Shopping <span style={{ fontWeight: 700, color: '#B91C1C' }}>Cart</span>
          </h1>
          {totalItems > 0 && (
        ...
        {items.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 20px', background: '#FDF8F3', border: '1px solid #E8E0D6' }}>
            <ShoppingBag size={56} style={{ margin: '0 auto 16px', opacity: 0.15, color: '#B91C1C' }} />
            <h2 style={{ fontSize: '1.3rem', fontWeight: 400, color: '#1A1A2E', marginBottom: '8px' }}>Your cart is empty</h2>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {items.map(item => (
                  <div key={`${item.id}-${item.size}`} style={{
                    ...
                    <div style={{ width: '80px', height: '100px', background: '#F1F5F9', flexShrink: 0, overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ width: '32px', textAlign: 'center', fontSize: '0.9rem', fontWeight: 500, color: '#1A1A2E' }}>{item.quantity}</span>
                      <button
                        onClick={() => updateQuantity(item.id, item.size, item.quantity + 1)}
                        style={{
                          width: '32px', height: '32px', border: '1.5px solid #E8E0D6', background: '#fff',
