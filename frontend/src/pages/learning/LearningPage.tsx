...
export default function LearningPage() {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#FDF8F3' }}>
        <div style={{ textAlign: 'center' }}>
          <Loader2 size={36} style={{ animation: 'spin 1s linear infinite', color: '#B91C1C' }} />
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#FDF8F3' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ color: '#1A1A2E', marginBottom: '8px' }}>Course Not Found</h2>
          <Link to="/dashboard" style={{ color: '#B91C1C', fontWeight: 600 }}>← Back to Dashboard</Link>
        </div>
      </div>
    );
  }
  ...
  const renderContent = (content: string) => {
    ...
    return lines.map((line, i) => {
      ...
      if (trimmed.startsWith('## ')) return <h2 key={i} style={{ fontSize: '1.4rem', fontWeight: 600, color: '#1A1A2E', marginTop: '28px', marginBottom: '12px', lineHeight: 1.3 }}>{trimmed.slice(3)}</h2>;
      if (trimmed.startsWith('### ')) return <h3 key={i} style={{ fontSize: '1.15rem', fontWeight: 600, color: '#1A1A2E', marginTop: '24px', marginBottom: '10px' }}>{trimmed.slice(4)}</h3>;
      if (trimmed.startsWith('> ')) return <blockquote key={i} style={{ borderLeft: '4px solid #1E3A8A', padding: '12px 20px', margin: '16px 0', background: '#FDF8F3', color: '#5A4A3A', fontStyle: 'italic', lineHeight: 1.7 }}>{trimmed.slice(2)}</blockquote>;
        ...
        return (
          <tr key={i} style={{ borderBottom: '1px solid #E8E0D6' }}>
            {cells.map((cell, ci) => isHeader
              ? <th key={ci} style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600, fontSize: '0.85rem', color: '#1A1A2E', background: '#F5F0EB' }}>{cell}</th>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#FDF8F3' }}>
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: '3px', zIndex: 100, background: '#E8E0D6' }}>
        <div style={{ height: '100%', background: 'linear-gradient(90deg, #B91C1C, #1E3A8A)', width: `${readProgress}%`, transition: 'width 0.1s' }} />
      </div>
      ...
      <aside style={{
                ...
                return (
                  <button key={sec._id} onClick={() => { setActiveSection(sec); setQuiz(null); setQuizResults(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                    style={{
                      ...
                      background: isActive ? '#FDF8F3' : 'transparent', border: 'none', borderLeft: isActive ? '3px solid #B91C1C' : '3px solid transparent',
                      cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'all 0.2s',
                    }}>
                    {isCompleted ? <CheckCircle size={16} color="#16a34a" /> : <Circle size={16} color={isActive ? '#B91C1C' : '#ddd'} />}
                    <span style={{ fontSize: '0.82rem', color: isActive ? '#B91C1C' : isCompleted ? '#16a34a' : '#4A4A4A', fontWeight: isActive ? 600 : 400, lineHeight: 1.4 }}>{sec.title}</span>
                  </button>
                );
              })}
            </div>
        ...
        <div style={{ padding: '16px 20px', borderTop: '1px solid #E8E0D6', background: '#FAFAF7' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: percentage >= 100 ? '#16a34a' : '#B91C1C' }}>{percentage}%</div>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ position: 'sticky', top: '3px', zIndex: 50, background: '#fff', borderBottom: '1px solid #E8E0D6', padding: '12px 32px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div style={{ fontSize: '0.85rem', fontWeight: 700, color: percentage >= 100 ? '#16a34a' : '#B91C1C', flexShrink: 0 }}>{percentage}%</div>
        </div>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 32px 100px' }}>
          {activeSection?.contentType === 'quiz' ? (
            <div>
              ...
              {!quiz && !quizResults && (
                <button onClick={() => loadQuiz(activeSection._id)} style={{ marginTop: '24px', padding: '14px 28px', background: '#B91C1C', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                  Start Quiz
                </button>
              )}
              {quiz && !quizResults && (
                <div style={{ marginTop: '32px' }}>
                    ...
                    <div key={q._id} style={{ marginBottom: '28px', padding: '20px', background: '#fff', border: '1px solid #E8E0D6', borderRadius: '6px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {q.options.map((opt, oi) => (
                          <label key={oi} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', border: `1.5px solid ${quizAnswers[qi] === oi ? '#B91C1C' : '#E8E0D6'}`, borderRadius: '4px', cursor: 'pointer', background: quizAnswers[qi] === oi ? '#FDF8F3' : '#fff', transition: 'all 0.2s' }}>
                  ...
                  <button onClick={submitQuiz} disabled={submittingQuiz || quizAnswers.includes(-1)}
                    style={{ padding: '14px 32px', background: quizAnswers.includes(-1) ? '#B8A88A' : '#B91C1C', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600, cursor: quizAnswers.includes(-1) ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: '2px solid #E8E0D6' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
              <button onClick={() => navigateSection('prev')} disabled={currentIdx <= 0}
                style={{ flex: 1, padding: '14px', background: currentIdx <= 0 ? '#f5f5f5' : '#fff', color: currentIdx <= 0 ? '#ccc' : '#1A1A2E', border: '1.5px solid #E8E0D6', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 500, cursor: currentIdx <= 0 ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                <ChevronLeft size={16} /> Previous
              </button>
              <button onClick={() => navigateSection('next')} disabled={currentIdx >= allSections.length - 1}
                style={{ flex: 1, padding: '14px', background: currentIdx >= allSections.length - 1 ? '#f5f5f5' : '#B91C1C', color: currentIdx >= allSections.length - 1 ? '#ccc' : '#fff', border: '1.5px solid transparent', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600, cursor: currentIdx >= allSections.length - 1 ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
