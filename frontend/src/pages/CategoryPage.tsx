    ...
    <div style={{ backgroundColor: '#FDF8F3', minHeight: '100vh' }}>
      ...
      <div className="container" style={{ padding: '60px 24px' }}>
        <div className="reveal" style={{ marginBottom: '48px' }}>
          ...
          <h1 style={{ fontSize: '2.5rem', fontWeight: 300, textTransform: 'capitalize', marginBottom: '8px', color: '#1A1A2E' }}>
          ...
          {products.map((product) => (
            <Link to={`/product/${product.id}`} key={product.id} style={{ textDecoration: 'none', color: 'inherit' }}>
              ...
              onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none'; }}>
                <div style={{ position: 'relative', paddingTop: '125%', overflow: 'hidden', background: '#F1F5F9' }}>
                  <img
                    src={product.image}
                    alt={product.name}
                    style={{
                ...
                <div style={{ padding: '20px' }}>
                  ...
                  <h3 style={{ fontSize: '0.95rem', fontWeight: 600, marginBottom: '6px', color: '#1A1A2E', marginTop: '4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{product.name}</h3>
                  <div style={{ fontSize: '1.1rem', fontWeight: 700, color: '#B91C1C' }}>₹{product.price.toLocaleString('en-IN')}</div>
                </div>
              </div>
            </Link>
          ))}
