        ...
        <header className="admin-header">
          ...
          <div className="header-actions">
            <div style={{ position: 'relative' }}>
              ...
              {showNotifications && (
                <div style={{ position: 'absolute', top: '100%', right: '0', marginTop: '12px', background: 'white', border: '1px solid #F1F5F9', borderRadius: '8px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', width: '320px', zIndex: 50 }}>
                  <div style={{ padding: '16px', borderBottom: '1px solid #F1F5F9', fontWeight: 600 }}>Live Notifications</div>
                  <div style={{ maxHeight: '350px', overflowY: 'auto' }}>
                    {notifications.map(notif => (
                      <div key={notif.id} style={{ padding: '16px', borderBottom: '1px solid #F1F5F9', fontSize: '0.875rem', cursor: 'pointer', animation: 'fadeIn 0.3s ease-in' }}>
            ...
            <div className="user-profile">
              <div className="avatar" style={{ backgroundColor: '#F1F5F9', color: '#1A1A2E', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600 }}>{user?.name?.[0] || 'A'}</div>
              {user?.name || 'Admin'}
            </div>
