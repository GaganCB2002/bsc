    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
        <div style={{ textAlign: 'center' }}>
          <Loader2 size={32} style={{ animation: 'spin 1s linear infinite', color: '#C47A6A' }} />
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '32px' }}>
        <span style={{ display: 'inline-block', fontSize: '0.6rem', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C9A84C', border: '1px solid rgba(201,168,76,0.3)', padding: '3px 12px', marginBottom: '8px' }}>Learning Dashboard</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '36px' }}>
        {[
          ...
          { icon: <TrendingUp size={22} />, label: 'Avg Progress', value: `${analytics?.averageProgress || 0}%`, color: '#C47A6A' },
          { icon: <Award size={22} />, label: 'Completed', value: analytics?.coursesCompleted || 0, color: '#16a34a' },
          { icon: <Clock size={22} />, label: 'Sections Done', value: analytics?.totalSectionsCompleted || 0, color: '#D4A574' },
        ].map((s, i) => (
      ...
      {inProgressCourses.length > 0 && (
        <div style={{ marginBottom: '36px' }}>
          <h2 style={{ fontSize: '1.2rem', fontWeight: 600, color: '#2C2826', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <BookMarked size={20} color="#C47A6A" /> Continue Learning
          </h2>
          <div style={{ display: 'grid', gap: '16px' }}>
            {inProgressCourses.map((p) => (
              <Link key={p.courseId._id} to={`/learning/${p.courseId._id}`}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#C47A6A', fontWeight: 600, fontSize: '0.8rem', flexShrink: 0 }}>
      ...
      <div style={{ marginBottom: '36px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h2 style={{ fontSize: '1.2rem', fontWeight: 600, color: '#2C2826' }}>All Courses</h2>
          <Link to="/learning" style={{ color: '#C47A6A', fontSize: '0.8rem', fontWeight: 600 }}>View All →</Link>
        </div>
        {courses.length === 0 ? (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #F0EBE5', padding: '24px' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '16px', color: '#2C2826' }}>Recent Activity</h3>
        ...
        {/* Completed Courses */}
        <div style={{ background: '#fff', border: '1px solid #F0EBE5', padding: '24px' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '16px', color: '#2C2826' }}>Completed Courses</h3>
